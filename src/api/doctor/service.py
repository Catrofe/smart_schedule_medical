...
class DoctorService:
    ...
    async def register_doctor(self, body: DoctorRegister, request: Request) -> DoctorOutput:
        ...
        raise RaiseScheduleMedical(request, status.HTTP_409_CONFLICT, "Doctor already exists")
    ...
    async def delete_doctor(self, id_doctor: int, request: Request) -> None:
        ...
        if not doctor_exist:
            raise RaiseScheduleMedical(request, status.HTTP_400_BAD_REQUEST, "Doctor not exists")
        await self._repository.delete_doctor(id_doctor)

    async def update_doctor(self, id_doctor: int, body: DoctorEdit, request: Request) -> DoctorOutput:
        doctor_exist = await self._repository.doctor_already_exist(id_doctor)
        if not doctor_exist:
            raise RaiseScheduleMedical(request, status.HTTP_400_BAD_REQUEST, "Doctor not exists")
