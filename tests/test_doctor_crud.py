...
@pytest.fixture
def create_user(create_especialty):
    client.post(
        URL_API,
        json={
...
def test_register_new_doctor_success(create_especialty):
...
def test_update_doctor_success(create_user):
    response = client.put(
        f"{URL_API}1", json={"email": "teste@123", "fullName": "Teste3333"}
    )

    assert response.status_code == 200
    assert response.json()["email"] == "teste@123"
    assert response.json()["fullName"] == "Teste3333"
